...
const estimationTurnaroundTrend = [
  { t: 'Run 1', v: 48.0 },
  { t: 'Run 2', v: 34.5 },
  { t: 'Run 3', v: 24.1 },
  { t: 'Run 4', v: 16.8 },
...
// High-Density Subcontractor Bid Pipeline Telemetry Timeline (Dope Fluid Waves)
const telemetryStream = [
  { time: '09:00', ops: 3820, latency: 14.8, stage: 'NCDOT & Commercial Letting Scrape' },
  { time: '10:00', ops: 4790, latency: 14.1, stage: 'DOT Plan Sheet & Line Item Extraction' },
  { time: '11:00', ops: 4210, latency: 13.9, stage: 'Barn GPS Haul & Distance Calculation' },
  { time: '12:00', ops: 5680, latency: 14.4, stage: 'Master Rate Sheet Unit Price Match' },
  { time: '13:00', ops: 5120, latency: 13.8, stage: 'Subcontractor Line-Item Estimate Calc' },
  { time: '14:00', ops: 6450, latency: 13.2, stage: 'Prime Contractor Plan-Holder Scrape' },
  { time: '15:00', ops: 5890, latency: 13.5, stage: 'Chief Estimator Email Verification' },
  { time: '16:00', ops: 6820, latency: 12.8, stage: 'Custom GC Proposal Email Draft' },
  { time: '17:00', ops: 6180, latency: 13.4, stage: 'Owner 1-Click Review & Approval Gate' },
  { time: '18:00', ops: 6450, latency: 13.2, stage: 'Approved Bid Package Dispatched' },
];
...
export function MetricsGrid() {
  ...
  const renderCardChart = (idx: number) => {
    ...
    if (idx === 0) {
      // Card 0: Subcontractor bid estimate curve (Stripe Blurple wavy gradient area)
      return (
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={bidVolumeTrend} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
            <defs>
                ...
                if (active && payload && payload.length) {
                  return (
                    <div className="rounded-[4px] bg-[var(--color-surface)] border border-[var(--color-border)] px-2.5 py-1 text-xs font-medium shadow-xs text-[var(--color-text-primary)]">
                      <span className="font-bold text-[#533AFD] dark:text-[#7A68FF]">${payload[0].value}k</span> Estimate
                    </div>
                  );
                }
                return null;
    ...
    if (idx === 1) {
      ...
      return (
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={gcMatchTrend} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
            <defs>
    ...
    if (idx === 2) {
      // Card 2: Contract Line-Item Pricing Accuracy (Amber wavy gradient area)
      return (
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={pricingAccuracySla} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
            <defs>
    ...
    // Card 3: Letting-to-Estimate Speed (Teal wavy gradient area)
    return (
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={estimationTurnaroundTrend} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
          <defs>
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm sm:text-[15px] font-bold tracking-tight text-[var(--color-text-primary)]">
                Subcontractor Bid Pipeline • Sub-50ms Handoff Telemetry
              </span>
            ...
            <p className="text-xs sm:text-sm text-[var(--color-text-secondary)] mt-1">
              Real-time context serialization &amp; throughput across Letting Scrape → Barn Distance → Rate Sync → GC Match → Owner Approval
            </p>
          </div>
